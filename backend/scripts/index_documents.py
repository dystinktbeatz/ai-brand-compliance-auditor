# This script reads PDF files from the backend/data directory, extracts and chunks their text content,
# generates vector embeddings using Azure OpenAI, and uploads them to an Azure AI Search index for later retrieval.
# It serves as a crucial step in building the knowledge base that the Brand Guardian AI will use to audit video content
# for compliance.

import os # For file paths and environment variables
import glob # For finding PDF files in the data directory
import logging # For logging progress and errors during the indexing process     
from dotenv import load_dotenv # For loading environment variables from a .env file. 
# The 'override=True' ensures that any existing environment variables will be overwritten by those in 
# the .env file, which is useful for ensuring that the correct configuration is used during development and testing.
load_dotenv(override=True) # Load environment variables from .env file, overriding existing ones if necessary.
# This ensures that the correct configuration is used during development and testing,
# and allows for easy management of sensitive information like API keys without hardcoding them in the codebase. 

# Document Loaders and Splitters
from langchain_community.document_loaders import PyPDFLoader # For loading PDF documents and extracting text.
from langchain_text_splitters import RecursiveCharacterTextSplitter # For splitting long text into smaller chunks 
# while maintaining context, which is crucial for effective embedding and retrieval. 
